...
class MP4Parser:
    ...
    def read_atom(self) -> Optional[MP4Atom]:
        ...
        size, atom_type = struct.unpack_from(">I4s", self.data, self.position)
        self.position += 8

        if size == 1:
            if self.position + 8 > len(self.data):
                return None
            size = struct.unpack_from(">Q", self.data, self.position)[0]
            self.position += 8

        if size < 8 or self.position + size - 8 > len(self.data):
            return None

        atom_data = self.data[self.position : self.position + size - 8]
...
class MP4Decrypter:
    ...
    def _get_key_for_track(self, track_id: int) -> bytes:
        ...
        key = self.key_map.get(track_id.to_bytes(4, "big"))
        if not key:
    ...
    def _process_stsd(self, stsd: MP4Atom) -> MP4Atom:
        ...
        """
        stsd_data = bytearray(stsd.data)
        parser = MP4Parser(memoryview(stsd_data[8:]))  # Skip version, flags, and entry count
        new_stsd_data = bytearray(stsd_data[:8])  # Keep version, flags, and entry count

        for atom in iter(parser.read_atom, None):
            new_sample_entry = self._process_sample_entry(atom)
            new_stsd_data.extend(new_sample_entry.pack())

        return MP4Atom(b"stsd", len(new_stsd_data) + 8, new_stsd_data)

    def _process_sample_entry(self, sample_entry: MP4Atom) -> MP4Atom:
        """
        ...
        """
        parser = MP4Parser(sample_entry.data)
        new_sample_entry_data = bytearray()

        for atom in iter(parser.read_atom, None):
            if atom.atom_type not in {b"sinf", b"schm", b"schi", b"tenc"}:
                new_sample_entry_data.extend(atom.pack())

        return MP4Atom(sample_entry.atom_type, len(new_sample_entry_data) + 8, new_sample_entry_data)
