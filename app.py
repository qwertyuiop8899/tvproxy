...
# --- VAVOO RESOLVE LOGIC (come plugin Kodi) ---
def getAuthSignature():
    ...
    req = requests.post('https://www.vavoo.tv/api/app/ping', json=_data, headers=_headers, timeout=10).json()
...
def gettsSignature():
    ...
    req = requests.post(url, data=vec, timeout=10).json()
    return req['response'].get('signed')

def resolve_vavoo_link(link):
    # Modalità API (POST) come default
    try:
        _headers = {
        ...
        url_api = "https://vavoo.to/mediahubmx-resolve.json"
        resp = requests.post(url_api, json=_data, headers=_headers, timeout=10)
        resolved_url = resp.json()[0]["url"]
        return resolved_url, None
    except Exception as e:
        # fallback: modalità diretta .ts con firma
        try:
            ts_url = "%s.ts?n=1&b=5&vavoo_auth=%s" % (link.replace("vavoo-iptv", "live2")[0:-12], gettsSignature())
            return ts_url, {"User-Agent": "VAVOO/2.6"}
...
def resolve_m3u8_link(url, headers=None):
    ...
    # --- PATCH: RISOLUZIONE AVANZATA VAVOO.TO ---
    if 'vavoo.to' in clean_url.lower():
        app.logger.info(f"URL Vavoo.to rilevato, risoluzione avanzata: {clean_url}")
        resolved_url, vavoo_headers = resolve_vavoo_link(clean_url)
        final_headers = {**current_headers, **extracted_headers}
        if vavoo_headers:
            final_headers.update(vavoo_headers)
        return {
...
def proxy_m3u():
    ...
    try:
        ...
        if not resolved_url.endswith('.m3u8'):
            app.logger.warning(f"URL risolto non è un M3U8: {resolved_url} - provo a restituire come singolo stream")
            # Se è un .ts o altro, restituisci una mini playlist M3U che punta al link risolto
            m3u = f"#EXTM3U\n#EXTINF:-1,Stream Diretto\n{resolved_url}\n"
            return Response(m3u, content_type="application/vnd.apple.mpegurl")
